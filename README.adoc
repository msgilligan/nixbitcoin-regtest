...
NOTE:: I have tested with Lima on macOS, but since the devShell should install Lima on your Linux host, the scripts should work on Linux, too. You will need to change `system` to `aarch64-linux` to `x86_64-linux` in at least one place. See https://github.com/msgilligan/nixbitcoin-regtest/issues/1[Issue #1] if you are interested in support for other VM managers.
...
The `bcli` alias is initialized by the devshell to `bitcoin-cli` configured to read a configuration from `./config/bitcoin.conf` which will correctly communicate with bitcoind in the guest.

To access with `bitcoin-cli`:

----
bcli getblockcount
bcli generatetoaddress 1 mwQA8f4pH23BfHyy4zf8mgAyeNu5uoy6GU
----

=== Electrs

Make sure at least one block has been mined by `bitcoind` with `generatetoaddress`. Then you can run:

----
./scripts/ElectrsRpc.java
----
...
+
----
./scripts/ClnRpc.java list-methods $RUNE
./scripts/ClnRpc.java getinfo $RUNE
----
